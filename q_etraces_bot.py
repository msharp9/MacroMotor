...
class QBot(sc2.BotAI):
    def __init__(self, action_space=None, learning_rate=0.01, reward_decay=0.9,
        e_greedy=0.9, trace_decay=0.8, model=None, title=1, record="records/q_etrace-record.txt"):
        self.MAX_WORKERS = 50
    ...
    def learn(self, s, a, r, s_):
        ...
        self.q_table += self.lr * error * self.eligibility_trace  # update
        # decay trace
    ...
    def on_end(self, game_result):
        ...
        if(game_result == Result.Defeat):
            self.learn(self.observation,self.action,-10000,'terminal')
        ...
        with open(self.record,"a") as f:
            #Model, Result, Time, Reward, Steps
        ...
        if self.model:
            ...
            self.q_table.to_csv('model/q_etrace_model.csv')
    ...
    async def on_step(self, iteration):
        ...
        if self.action is not None:
            self.learn(self.observation, self.action, self.reward, _observation)
            self.tot_reward += self.reward
            self.reward = -1 # penalize longer games
        self.observation = _observation
        self.action = self.choose_action(_observation)
        try:
    ...
    def intel(self):
        ...
        # without a NN to interpret the image the state size is too large
        # print(self.units(), self.known_enemy_units, len(self.units()), len(self.known_enemy_units))
        data = [worker_weight, plausible_supply, population_ratio,
            vespene_ratio, mineral_ratio, len(self.units()), len(self.known_enemy_units)]
        # return ','.join([str(_) for _ in data])
        return "{:1.2f},{:1.2f},{:1.2f},{:1.2f},{:1.2f},{},{}".format(*data)
...
if __name__ == "__main__":
    ...
    #         Bot(Race.Protoss, QBot(model='model/Q_etrace.csv',
    #             # record="Q-record-vs-easy.txt",
    #             learning_rate=0.1, reward_decay=0.9, e_greedy=0.1, trace_decay=0.95, title=1)),
    #         # Bot(Race.Protoss, QBot(model='model/Qmodel.csv', learning_rate=0.01, e_greedy=0.9*(episode/100), title=1)),
    #         # Human(Race.Terran),
    ...
    for episode in range(1):
        ...
        run_game(maps.get("AbyssalReefLE"), [
            Bot(Race.Protoss, QBot(model='model/q_etrace.csv',
                # record="Q-record-vs-easy.txt",
                learning_rate=0.05, reward_decay=0.9, e_greedy=0.95, trace_decay=0.7, title=1)),
            # Bot(Race.Protoss, QBot(model='model/Qmodel.csv', learning_rate=0.01, e_greedy=0.9*(episode/100), title=1)),
            # Human(Race.Terran),
            # Computer(Race.Protoss, Difficulty.Easy),
            Computer(Race.Protoss, Difficulty.Medium),
